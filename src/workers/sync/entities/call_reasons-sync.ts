/**
 * Sync worker for Call reason definitions
...
export class CallReasonsSyncWorker extends BaseSyncWorker {
  readonly endpointName = 'call_reasons';
  readonly stEndpointPath = 'https://api.servicetitan.io/jbce/v2/tenant/3222348440/call-reasons';
  readonly rawTable = 'raw.st_call_reasons';
  readonly masterTable = 'master.call_reasons';
  ...
  protected async transformToMaster(batchId: string): Promise<{ inserted: number; updated: number }> {
    const result = await this.db.query(`
      ...
      SELECT 
        ...
        (payload->>'isLead')::boolean as is_lead,
